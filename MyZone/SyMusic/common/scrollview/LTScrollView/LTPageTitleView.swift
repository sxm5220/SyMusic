...
@objc public class LTPageTitleView: UIView {
    ...
    private var atIndex: Int = -1 //在第几个位置
    ...
    @objc public init(frame: CGRect, titles: [String], atIndex: Int, isNotice: Bool, layout: LTLayout) {
        self.titles = titles
...
extension LTPageTitleView {
    ...
    func setupButtonsLayout() {
        ...
        for index in 0..<titles.count {
            ...
            button.badgeEdgeInsets = UIEdgeInsets(top: 20, left: 10, bottom: 0, right: 15)
            upX = button.frame.origin.x + subW + layout.titleMargin
            if index == atIndex && self.isNotice{
                button.badgeValue = "1"
            }else if index == atIndex && !self.isNotice {
                button.badgeValue = ""
            }
...
extension LTPageTitleView {
    ...
    private func glt_setupScrollToIndex(_ index:inout Int) {
        if index >= titles.count {
            SyPrint("超过最大数量限制, 请正确设置值, 默认这里取第一个")
            index = 0
        }
...
extension LTPageTitleView {
    ...
    @discardableResult
    private func subButton(frame: CGRect, flag: Int, title: String?, parentView: UIView) -> SyBadgeButton {
        let button = SyBadgeButton(type: .custom)
        button.frame = frame
        button.tag = flag
