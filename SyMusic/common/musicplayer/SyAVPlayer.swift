...
protocol SyAVPlayerDelegate : class {
...
//全局通知 name
let SyAVPlayerState = "SyAVPlayerState"
let CurrentPlayUrl = "CurrentPlayUrl"
let PlayType = "PlayType"

class SyAVPlayer: NSObject {
    
    private static var _sharedInstance: SyAVPlayer?
    // player 单例
    class func getSharedInstance() -> SyAVPlayer {
        guard let instance = _sharedInstance else {
            _sharedInstance = SyAVPlayer()
    ...
    var isIntroductionDetail: Bool = false
    var payStatus: Bool = false //课程是否付费
    //当你播放的不仅仅是音乐的话，而是多种类型的音频 例如： 试听，问题回答，音乐连续播 等等
    // 这时一定要提前分好类  会好一点
    var playType: SyAVPlayerType = SyAVPlayerType.PlayTypeTry  //1 景点，线路  2 专题播放  3 试听  4 回答问题 5 其他
    ...
    var model: SyMusicsModel?
    var imageView = UIImageView() //为了设置锁屏封面
    // 音频播放数组  例如 多个需要连续播放的音频 用比较好
    var musicArray : [SyMusicsModel] = []
    fileprivate var lastRow = -1
    fileprivate var artWork: MPMediaItemArtwork?
    
    func getLrcMs(_ lrcName: String?) -> [SyLrcModel] {
        if lrcName == nil {
            return [SyLrcModel]()
        }
        guard let path = Bundle.main.path(forResource: lrcName, ofType: nil) else { return [SyLrcModel]() }
        var lrcContent = ""
        do {
    ...
    func getCurrentLrcM(_ currentTime: TimeInterval, lrcMs: [SyLrcModel]) -> (row: Int, lrcM: SyLrcModel?) {
        for i in 0..<lrcMs.count {
    ...
    fileprivate var musicMessageM = SyMusicMessageModel()
    func getMusicMessageModel() -> SyMusicMessageModel {
    ...
    class func dataSource(_ result: ([SyMusicsModel])->()) {
        guard let path = Bundle.main.path(forResource: "SyMusics", ofType: "plist") else {
            result([SyMusicsModel]())
            return
        }
        ...
        var musicMs = [SyMusicsModel]()
        for dic in array {
            let musicM = SyMusicsModel()
            musicM.initWithDictionary(dic: dic as! NSDictionary)
    ...
    @objc func playMusicFinished(){
        ...
        if (self.playType == SyAVPlayerType.PlayTypeLine ||
            self.playType == SyAVPlayerType.PlayTypeSpecial) {
            if SyAVPlayer.getSharedInstance().isIntroductionDetail {
...
extension SyAVPlayer {
    ...
    func playMusic(url :String, type :SyAVPlayerType){
    ...
    //播放当前这首（单曲循环）
    func current() {
        if self.playType == SyAVPlayerType.PlayTypeLine || self.playType == SyAVPlayerType.PlayTypeSpecial {
...
//            }
    ...
    func updateCurrentPlayState(state : AVPlayerPlayState){
        if self.currentUrl != nil {
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: SyAVPlayerState), object: nil, userInfo: [SyAVPlayerState : state,CurrentPlayUrl : self.currentUrl!,PlayType : self.playType])
        }else {
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: SyAVPlayerState), object: nil, userInfo: [SyAVPlayerState : state,CurrentPlayUrl : "",PlayType : self.playType])
        }
    }
