...
let BGWIDTH = screenWidth()

let KEY_WINDOW_HEIGHT = screenHeight()

typealias ResultClosure = (String, String, String) -> Void

class SyCityChooseView: UIView,  UIPickerViewDelegate , UIPickerViewDataSource {
    ...
    fileprivate lazy var cancleBtn: UIButton = {
        let btn = buttonWithTitleFrame(frame: CGRect.init(x: 10, y: 5, width: 50, height: BGHEIGHT - PICKERHEIGHT - 10), title: strCommon(key: "sy_canel"), titleColor: UIColor.black, backgroundColor: UIColor.clear, cornerRadius: 5, tag: 10, target: self, action: #selector(actionBtnClick(sender:)))
        return btn
    }()
    
    fileprivate lazy var sureBtn: UIButton = {
        let btn = buttonWithTitleFrame(frame: CGRect.init(x: BGWIDTH - 60, y: 5, width: 50, height: BGHEIGHT - PICKERHEIGHT - 10), title: strCommon(key: "sy_sure"), titleColor: UIColor.black, backgroundColor: UIColor.clear, cornerRadius: 5, tag: 11, target: self, action: #selector(actionBtnClick(sender:)))
        return btn
    }()
    ...
    func showPickerView() -> Void {
        UIView.animate(withDuration: 0.3) {
            ...
            self.bgView.frame = CGRect.init(x: 0, y: KEY_WINDOW_HEIGHT - BGHEIGHT - (isIphoneX() ? 100 : 30), width: BGWIDTH, height: BGHEIGHT + (isIphoneX() ? 100 : 30))
        }
    }
    ...
    func hidePickerView() -> Void {
        UIView.animate(withDuration: 0.3, animations: {
            self.bgView.frame = CGRect.init(x: 0, y: KEY_WINDOW_HEIGHT, width: BGWIDTH, height: BGHEIGHT + (isIphoneX() ? 100 : 30))
            self.alpha = 0
        }) { (finished) in
