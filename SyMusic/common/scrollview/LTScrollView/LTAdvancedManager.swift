...
public class LTAdvancedManager: UIView {
    ...
    private var atIndex: Int = -1
    private var isNotice: Bool = false
    ...
    @objc public init(frame: CGRect, viewControllers: [UIViewController], titles: [String], atIndex: Int, isNotice: Bool, currentViewController:UIViewController, layout: LTLayout, titleView: LTPageTitleView? = nil, headerViewHandle handle: () -> UIView) {
...
extension LTAdvancedManager {
    private func setupTitleView() -> LTPageTitleView {
        let titleView = LTPageTitleView(frame: CGRect(x: 0, y: 0, width: bounds.width, height: layout.sliderHeight), titles: titles, atIndex: self.atIndex, isNotice: self.isNotice, layout: layout)
        return titleView
    }
}
