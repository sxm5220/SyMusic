...
extension SyMusicPlayVC: SyAudioPlayerViewDelegate {
    ...
    func preMusicActionFunc() {
        if SyAVPlayer.getSharedInstance().playType == SyAVPlayerType.PlayTypeLine || SyAVPlayer.getSharedInstance().playType == SyAVPlayerType.PlayTypeSpecial {
            ...
            if index < 0 {
                progressHUDShowWarningWithStatus(status: strCommon(key: "sy_first_song_title"))
                return
            }
    ...
    func nextMusicActionFunc() {
        if SyAVPlayer.getSharedInstance().playType == SyAVPlayerType.PlayTypeLine || SyAVPlayer.getSharedInstance().playType == SyAVPlayerType.PlayTypeSpecial {
            ...
            if index >= count {
                progressHUDShowWarningWithStatus(status: strCommon(key: "sy_last_song_title"))
                return
            }
            SyAVPlayer.getSharedInstance().changeTheMusicByIndex(index: index)
            self.changePlay()
    ...
    func playListActionFunc() {
        ...
        listView.myClosure = { (id: String) -> Void in
            for i in 0..<SyAVPlayer.getSharedInstance().musicArray.count {
                let model: SyMusicsModel = SyAVPlayer.getSharedInstance().musicArray[i]
                if id == model.id {
...
class SyMusicPlayVC: SyBaseVC {
    ...
    var dataCourseArray: [SyMusicsModel] = [SyMusicsModel]()
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        let v = SyPlayerShowView(frame: CGRect(x: 2, y: screenHeight(), width: screenWidth() - 4, height: 50), isShow: true, categoryId: self.categoryId)
...
//        self.playView.tView?.stopTimer()
    ...
    fileprivate func musicListDataSource(isRefresh: Bool) {
        SyAVPlayer.dataSource { (models: [SyMusicsModel]) in
            for i in 0..<models.count {
                let m = models[i]
                if self.categoryId == m.category {
                    self.dataCourseArray.append(m)
                }
            }
